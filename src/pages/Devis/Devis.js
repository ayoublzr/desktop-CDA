...
function Devis() {
    ...
    useEffect(() => {
      axios.get('http://localhost:3003/api/devis-list')
        .then(response => {
    ...
    const handleDelete = (devisId) => {
      axios.delete(`http://localhost:3003/api/deletedevis/${devisId}`)
        .then(response => {
    ...
    // Regrouper les devis avec le même ID
    devisList.forEach(devis => {
      const existingDevis = groupedDevisList.find(item => item.DevId === devis.DevId);
      if (existingDevis) {
              ...
              <>
                <tr key={devis.DevId}>
                  ...
                  <td rowSpan={devis.products.length}>
                    <button className="btn btn-danger" onClick={() => handleDelete(devis.DevId)}>Supprimer</button>
                  </td>
                </tr>
